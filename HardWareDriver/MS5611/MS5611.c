...
float Filter_Altitude=0.0f;//update20170315气压计和超声波和加速度计滤波得到的高度，以后高度就用这个值
uint8_t Ultra_IsUseful = 0;//update20161227:新增:是否只使用超声波数据标识,==1的时候不使用气压计,这个变量的值有超声波数据健康度决定
float Filter_Altitude_D = 0;//高度变化率，即垂直速度，单位0.01m/s//update20170315气压计和超声波和加速度计滤波得到的垂直速度，以后就用这个值
float debug_flag = 0;
...
// update20170110:添加对高度数据是否合理的判断，偏差太大的数据则剔除
	...
	if(Ultra_IsUseful)
	{
		...
		{
		    val = 0.01f * val + 98.0f * Position_Z + 0.01f * MS5611_Altitude;
		    debug_flag -= 50;
		}
	}
	else
	{
			val = 0.15f * val + 85.0f * Position_Z;
	}
