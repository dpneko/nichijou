...
float  Target_Roll,Target_Pitch,Target_Yaw,Tartget_hight = 0.4;
...
//20161125从HeightPID裁剪的代码
	...
	pidSetTarget_Measure(&Climb,ClimbTarget,Filter_Altitude_D);//update20170312
...
//使用三级串联pid
	...
	// pidSetPID(&Z_Speed, 1.0f, 0.0f, 0.0f);
	...
	// }
	// pidSetTarget_Measure(&AutoHigh_THR,Z_Speed.PID_out,Filter_Altitude_D);//update20170312
	...
	// }
	// pidSetTarget_Measure(&Climb,AutoHigh_THR.PID_out,(Motion_Accz) * 100.0f);
...
void GetStartInfo(void)
{
	GroundAltitude = (GroundAltitude * GetStartInfoCnt + Filter_Altitude) / (GetStartInfoCnt + 1 );//启动时记录地面高度
...
void Get_Tartget_RPY(void)
{
	...
	if(GCSControl_Leftward)		Target_Roll-=5;
	if(GCSControl_Rightward)	Target_Roll+=5;
	if(GCSControl_Forward)		Target_Pitch+=5;
	if(GCSControl_Backward)		Target_Pitch-=5;
...
{//TODO: 定高模式函数还没开始写
	...
	if (PWM_Input_CH3 > (int16_t)(MINTHROTTLE + (MAXTHROTTLE - MINTHROTTLE) / 10))
	{ //遥控器油门大于10% 定高才会起作用，防止误动作引起电机转动
	    Quad_THR += Height_PID(Filter_Altitude/100.0f);
	}
	else
	{
	    pidReset(&Climb);
